...
1. **Install the .exe** — build with `npm run tauri:build:win` (in `v2/desktop/ai/vibecodeworker`), then run the installer from `src-tauri/target/release/bundle/nsis/` (`.exe`; `.msi` beside it).
