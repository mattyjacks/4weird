...
export function CheckoutControl() {
  ...
  return <section className="rounded-2xl border border-white/10 bg-white/[.04] p-6"><h2 className="text-xl font-bold">Buy Vibe Coins</h2><p className="mt-2 text-sm text-slate-400">Enter the Shopify variant ID for a coin pack.</p><div className="mt-4 flex gap-2"><input aria-label="Shopify variant ID" inputMode="numeric" value={variantId} onChange={(event) => setVariantId(event.target.value)} placeholder="Variant ID" className="min-w-0 flex-1 rounded-lg border border-white/15 bg-black/30 px-3 py-2" disabled={busy} /><button type="button" disabled={!valid || busy} onClick={start} className="rounded-lg bg-cyan-300 px-4 py-2 font-semibold text-slate-950 disabled:opacity-40">{busy ? "Opening…" : "Checkout"}</button></div>{message && <p role="status" className="mt-3 text-sm text-amber-200">{message}</p>}</section>;
}
