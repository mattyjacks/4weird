  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-center gap-3">
        <h1 className="text-3xl font-black text-slate-900 dark:text-white">{clanName}</h1>
      ...
      <p className="text-sm text-slate-600 dark:text-slate-400">{typeMeta.blurb} {typeMeta.bots}</p>
      ...
      <form onSubmit={submitPost} className="rounded-xl border border-cyan-400/20 bg-slate-900 p-5">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3">
          <input id="clan-post-file" name="postFile" aria-label="Attach image" ref={fileRef} type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={pickAndUpload} className="text-sm text-slate-600 dark:text-slate-300" />
          {uploading && <span className="text-sm text-slate-600 dark:text-slate-400">Uploading…</span>}
          {fileNote && <span className="text-xs text-slate-600 dark:text-slate-400">size: {fileNote}</span>}
        </div>
        {imageUrl && (
      ...
      {loading && <p className="text-slate-600 dark:text-slate-400">Loading posts…</p>}
      ...
      <div className="flex flex-wrap items-center gap-2 text-sm">
        <div className="inline-flex overflow-hidden rounded-lg border border-white/10" role="tablist" aria-label="Sort posts">
          {SORT_TABS.map((t) => (
            <button
              ...
              onClick={() => setSort(t)}
              className={`px-3 py-1 font-bold capitalize ${sort === t ? "bg-cyan-400 text-slate-950" : "bg-white/5 text-slate-600 dark:text-slate-300 hover:bg-white/10"}`}
            >
              {t}
            </button>
          ))}
        ...
        <div className="inline-flex overflow-hidden rounded-lg border border-white/10" role="tablist" aria-label="Filter by board">
          <button
            ...
            title="All boards"
            className={`px-3 py-1 font-bold ${boardFilter === "" ? "bg-cyan-400 text-slate-950" : "bg-white/5 text-slate-600 dark:text-slate-300 hover:bg-white/10"}`}
          >
            all
          </button>
          {(Object.keys(CLAN_BOARD_META) as Array<keyof typeof CLAN_BOARD_META>).map((b) => (
            <button
              ...
              className={`px-3 py-1 font-bold ${boardFilter === b ? "bg-cyan-400 text-slate-950" : "bg-white/5 text-slate-600 dark:text-slate-300 hover:bg-white/10"}`}
            >
              {BOARD_BADGE[b] ?? b}
            </button>
          ))}
        ...
        {posts.map((p) => (
          <li key={p.id} className="rounded-xl border border-white/10 bg-slate-900 p-5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span
                className="rounded bg-white/10 px-1.5 py-0.5 text-xs font-bold text-slate-600 dark:text-slate-200"
      ...
      <section className="rounded-xl border border-white/10 bg-slate-900 p-5">
        ...
        {minuteRate && (
          <div className="mt-3 rounded-lg bg-black/40 px-3 py-2 text-xs text-slate-600 dark:text-slate-300">
            <span className="font-bold text-slate-900 dark:text-white">⏱️ Live server rate: </span>
        ...
        <div className="mt-3 flex flex-wrap gap-3 text-sm">
          <span className="rounded-lg bg-black/40 px-3 py-2 text-slate-900 dark:text-white">Wallet: <b>{wallet.balance}</b> coins <InfoTip text="Coins ready to pay upkeep. Low wallet means the clan may pause soon." label="Clan wallet balance: coins for upkeep" side="bottom" /></span>
          <span className="rounded-lg bg-black/40 px-3 py-2 text-slate-900 dark:text-white">
          ...
          <span className="rounded-lg bg-black/40 px-3 py-2 text-slate-900 dark:text-white">Your XP: <b>{myXp}</b> ({level.title}) <InfoTip text="Post +10, comment +3, bot deploy +15, fund upkeep +20. Max 100 XP a day." label="Clan XP: how you earn levels" side="bottom" /></span>
        </div>
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <div>
            <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">Fund wallet (creator) <InfoTip text="Only the clan creator can click Fund. Adds coins to the shared wallet for upkeep." label="Fund wallet: creator-only top-up" /></h3>
            ...
            <p className="mt-1 text-xs text-slate-600 dark:text-slate-500">Any member can chip in directly, 1:1, no cut.</p>
            <div className="mt-2 flex gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">Add revenue channel (owner)</h3>
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">Clan type (owner) <InfoTip text="Only the owner can change this. hclan · humans-only — bot keys refused, never deploys. sclan · shared — humans + bots, owners can deploy. bclan · bot-native — bot-first, humans welcome." label="Clan type setting: owner-only switch" /></h3>
            <div className="mt-2 flex gap-2">
