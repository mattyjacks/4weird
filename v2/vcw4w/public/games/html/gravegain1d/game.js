...
(function () {
    ...
    var CLASSES = {
        ...
        sapper: { id: 'sapper', name: 'Forge Sapper', emoji: '🛠️', hp: 30, atk: 4, def: 3, spd: 1, range: 2,
            ability: { name: 'Stone Oath', cd: 8, desc: 'Shield 6 for 2 ticks + heal 4' },
            blurb: 'Borin needs hands that fix under fire. Carry the big charge, light the dark, break the golems.' },
        runner: { id: 'runner', name: 'Grove Runner', emoji: '🌿', hp: 16, atk: 6, def: 0, spd: 4, range: 3,
            ability: { name: 'Hex Nova', cd: 7, desc: '3 dmg to all in range 4 + slow' },
            blurb: 'Aelindra\u2019s pick: fast feet, quiet steps, field-blessed. Outrun the swarm, cut the anchors first.' }
    };

    var KINDS = {
        shambler: { name: 'Risen Shambler', emoji: '🧟', hp: 12, atk: 3, def: 0, spd: 2, aggro: 6, xp: 3, gold: 2 },
        swarm: { name: 'Skull Swarm', emoji: '💀', hp: 5, atk: 4, def: 0, spd: 4, aggro: 8, xp: 2, gold: 1 },
        brute: { name: 'Zed Brute', emoji: '👹', hp: 26, atk: 5, def: 1, spd: 1, aggro: 5, xp: 5, gold: 4 },
    ...
    var SECTORS = [
        { name: 'Crash Flats', theme: '#1e293b', sky: '#0b0f1a',
            ...
            spawn: [['shambler', 12]],
            boss: { name: 'James Wright, the First Risen', emoji: '🪦', hp: 40, atk: 4, def: 0, mech: 'wright' },
            codex: 'helmet' },
        { name: 'Whisper Groves', theme: '#064e3b', sky: '#02120d',
            ...
            boss: { name: 'Mirathiel, Echo of the Elder', emoji: '👻', hp: 55, atk: 5, def: 0, mech: 'mirathiel' },
            codex: 'compact' },
        { name: 'Sparkite Cut', theme: '#7c2d12', sky: '#1c0a02',
            ...
            boss: { name: 'The Vault Warden, Forge-Golem', emoji: '🗿', hp: 80, atk: 6, def: 2, mech: 'warden' },
            codex: 'sparkite' },
        { name: 'Ash Gate', theme: '#7f1d1d', sky: '#1c0505',
            ...
            spawn: [['shambler', 6], ['brute', 4], ['necro', 2], ['swarm', 4]],
            boss: { name: 'Karguk, Blood-Bound Berserker', emoji: '😈', hp: 110, atk: 7, def: 1, mech: 'karguk' },
            codex: 'guy' },
        { name: 'Relay Approach', theme: '#4c1d95', sky: '#0d021c',
            ...
            boss: { name: 'Gate Titan, Herald of the Necrogenesis', emoji: '👹', hp: 150, atk: 8, def: 1, mech: 'titan' },
            codex: 'good' }
    ];
    ...
    function newRun(seed, classId, mode) {
        ...
        var s = {
            ...
            level: 1, xp: 0, gold: 0, potions: 1, kills: 0, ticks: 0,
    ...
    function enemyAct(s, e, events) {
        ...
        function stepOk(nx) {
            if (nx < 0 || nx >= WORLD_LEN) return false;
            var f = foeAt(s, nx);
            if (f && f !== e) return false;
            if (Math.abs(nx - s.x) < 0.6) return false;
            return true;
        }
        function meleeHit(mult) {
            var dmg = Math.max(1, (e.atk * (mult || 1)) - s.def + roll(s));
        ...
        if (e.kind === 'necro') {
            ...
            if (e.t % 5 === 0) {
                s.enemies.push({ id: nextId++, kind: 'shambler', x: Math.max(0, s.x - 2),
                    hp: 12, maxHp: 12, atk: 3, def: 0, spd: 2, aggro: 99,
        ...
        if (e.kind === 'brute') {
            if (e.t % 4 === 0 && d <= 2) { meleeHit(8 / Math.max(1, e.atk) * e.atk); return; } // slam ≈8
    ...
    function bossAct(s, e, events, d, toward, stepOk, meleeHit) {
        ...
        if (def === 'wright') {
            if (e.t % 4 === 0 && d <= 6) { // Crash Charge telegraph then rush
                e.x = s.x + 1;
                meleeHit(6 / Math.max(1, e.atk) * e.atk + 1);
                events.push({ t: 'charge', x: e.x });
                return;
            }
            if (e.t % 6 === 0) {
                for (var i = 0; i < 2; i++) {
                    s.enemies.push({ id: nextId++, kind: 'shambler', x: Math.min(WORLD_LEN - 1, e.x + 2 + i),
                        hp: 12, maxHp: 12, atk: 3, def: 0, spd: 2, aggro: 99, emoji: '🧟',
            ...
            if (d <= 5) { // Thorn Volley pierces 2 DEF
            ...
            if (!e.half && e.hp <= e.maxHp / 2) { e.half = true; e.def += 2; events.push({ t: 'enrage', name: e.name }); }
            if (e.t % 4 === 0 && d <= 2) { meleeHit(8 / Math.max(1, e.atk) * e.atk + 1); return; }
            if (d > 1) { var nxw = e.x + toward; if (stepOk(nxw)) e.x = nxw; }
            else meleeHit(1);
            return;
        } else if (def === 'karguk') {
            if (e.t % 5 === 0) { // Down-the-Line Charge: hits wherever you stand
                var dmg2 = Math.max(1, 5 - s.def + roll(s));
            ...
            if (e.t % 5 === 0) { // Array Pulse
                damage(s, null, Math.max(1, 4 - s.def + roll(s)), true, events);
                s.enemies.push({ id: nextId++, kind: 'swarm', x: Math.min(WORLD_LEN - 1, s.x + 3),
                    hp: 5, maxHp: 5, atk: 4, def: 0, spd: 4, aggro: 99, emoji: '💀',
    ...
    function tick(s, action) {
        ...
        try {
            if (action.potion) drinkPotion(s, events);
                ...
                var nx = s.x + 1;
                var cap = s.gateOpen ? WORLD_LEN - 1 : Math.min(s.gateX + 2, WORLD_LEN - 1);
        ...
        if (!s.boss && !s.gateOpen && s.x >= s.gateX - 10 && s.sector < 5 + s.cycle * 5) {
            var b = spawnBoss(s);
        ...
        for (var ai = 0; ai < s.enemies.length; ai++) {
            var m = s.enemies[ai];
            if (m.isBoss && m.hp > 0 && m.mech === 'karguk') aura = 2; // War Drum
        }
        for (var i = 0; i < s.enemies.length; i++) {
            var e = s.enemies[i];
        ...
        if (s.gateOpen) {
            ...
            if (pure && s.sector === 4) {
                ...
                // Advance: heal a little, +1 potion, next sector spawns.
                ...
                // Endless: next cycle chunk.
                s.hp = Math.min(s.maxHp, s.hp + Math.ceil(s.maxHp * 0.3));
                s.potions = Math.min(3, s.potions + 1);
                var next = s.sector + 1;
                if (next % 5 === 0 && next > 0 && s.sector >= 4) { s.cycle += 1; }
                s.x = (s.sector + 1) * SECTOR_LEN;
                if (s.x >= WORLD_LEN) { s.x = WORLD_LEN - 1; }
                spawnSector(s, s.sector + 1 > 4 ? 4 : s.sector + 1);
                // Rebase endless sectors forward so the line never ends visually.
                s.gateX = s.x + SECTOR_LEN - 1;
                events.push({ t: 'sector', sector: s.sector });
            }
        }
        return events;
    ...
    function onEvents(events) {
        try {
            for (var i = 0; i < events.length; i++) {
                ...
                else if (e.t === 'boss') { sfx('boss'); showBanner('💀 ' + e.name.toUpperCase(), bossBrief(e.name), null); }
    ...
    function render() {
        try {
            ...
            var cam = clamp(s.x - 12, 0, Math.max(0, WORLD_LEN - VIEW_TILES));
            ...
            var prog = clamp(s.x / (WORLD_LEN - 1), 0, 1);
