/* GraveGain2D AAA — 30 announcer.
   ...
   Exposes AAA.announce(main, sub) for other modules (boss slays). */
(function () {
    'use strict';
    ...
    AAA.ready((game) => {
        ...
        // Every melee swing feeds the combo by its confirmed hit count.
        // (AoE ability kills resolve through runKills and are picked up
        // by the streak poll below, so no kill goes uncounted.)
        AAA.wrap(game, 'triggerMeleeSwing', (orig, ...args) => {
            const out = orig(...args);
            try {
                const hits = typeof out === 'number' ? out : 0;
                if (hits > 0) {
                    combo += hits;
                    ...
                    AAA.emit('hit', { combo, killed: false });
                }
