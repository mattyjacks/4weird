/* =========================================================================
 * GraveGain5D -- v2 content-mode gore overlay (slug: gravegain5d ONLY)
 * -------------------------------------------------------------------------
 * v2-native file. Lives OUTSIDE the parity-locked bundle trees (never edit
 * those). Injected into the *generated* bundle (public/games/gravegain5d/)
 * by scripts/sync-game-bundles.mjs -- same pattern as gore-gravegain3d.js,
 * gravegain4d-gore.js, gore-gravegain2d.js, and gore-lastwordszombies.js.
 * Until that injection block lands, load this file with a plain <script>
 * tag (see wiring note at the bottom of this header).
 *
 ...
 * Exposes window.GraveGain5DGore { getMode, setMode, spawnKill, VERSION }.
 *
 * Kill sources:
 *   - engine kill counter (window.GraveGain5DGame || GraveGainGame || gg)
 *   - golf-strike kills: hole-outs on hazard foes. The golf core
 *     (window.GraveGain5DGolf) emits 'hole-out' / 'hole-complete' events per
 ...
 *     GraveGain5DGore.attachRound(round).
 *
 ...
 *   scripts/sync-game-bundles.mjs needs a gravegain5d block mirroring the
 *   gravegain4d one:
 *     if (slug === "gravegain5d") {
 *       const goreSrc = join(root, "public", "games", "html", "gravegain5d-gore.js");
 *       if (existsSync(goreSrc) && !html.includes("gravegain5d-gore.js")) {
 *         const tag = `<script src="/games/html/gravegain5d-gore.js"></script>`;
...
(function () {
    'use strict';
    if (window.GraveGain5DGore) return;
    ...
    function game() {
        try {
            return window.GraveGain5DGame || window.GraveGainGame || window.gg || null;
        } catch (_) { return null; }
    }

    function golf() {
        try { return window.GraveGain5DGolf || null; } catch (_) { return null; }
    }
    ...
    function spawnKillAt(nx, ny, count, extra) {
        ...
        try {
            window.dispatchEvent(new CustomEvent(GORE_EVENT, {
                detail: { game: 'gravegain5d', mode: state.mode, kills: count }
            }));
        } catch (_) {}
    }
    ...
    function attachGolfRound(round) {
        try {
            ...
            if (round.__gg5dGoreAttached) return true;
            round.__gg5dGoreAttached = true;
    ...
    function hookGolf() {
        try {
            ...
            if (g.__gg5dGoreWrapped) return true;
    ...
    var api = null;
    try { api = window.GraveGain5DGore || null; } catch (_) {}

    try {
        window.GraveGainMods = window.GraveGainMods || [];
        window.GraveGainMods.push({ name: 'gravegain5d-gore', version: VERSION, init: function () { return api; } });
    } catch (_) {}
