    ...
    Input4D.prototype.attach = function (container) {
        ...
        window.addEventListener('keydown', function (e) {
            ...
            if (e.code === 'KeyE' || e.code === 'KeyR') {
                var ball = self.putt && self.putt.ball;
                if (ball) {
                    var direction = e.code === 'KeyE' ? 1 : -1;
                    ball.w = Math.max(self.putt.wMin || 0, Math.min(self.putt.wMax || 3, ball.w + direction));
                }
            }
        ...
        document.addEventListener('mousemove', function (e) {
            ...
            if (self.putt && typeof self.putt.setAim === 'function') {
                var cp = Math.cos(self.pitch);
                self.putt.setAim(Math.sin(self.yaw) * cp, Math.sin(self.pitch), Math.cos(self.yaw) * cp, 0);
            }
