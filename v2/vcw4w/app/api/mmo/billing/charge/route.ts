...
/**
 * POST /api/mmo/billing/charge — session billing QUOTE stub (no writes).
 *
...
export async function POST(req: Request) {
  ...
  const { data: server, error: serverError } = await supabase
    ...
    .maybeSingle();
  if (serverError) return dbFail("POST /api/mmo/billing/charge", serverError, "Unable to load server.");
  ...
  if (balanceError) return dbFail("POST /api/mmo/billing/charge", balanceError, "Unable to check balance.");
