...
export async function GET() {
  try {
    ...
    // Best-effort liveness: each probe is guarded so one bad pod can never
    // fail the whole lane.
    ...
    if (!msg.includes("During prerendering")) {
      console.error("[api] GET /api/vcw/autoplay/mine unhandled", msg.slice(0, 200));
    }
    return fail("Unable to list autoplay remotes.", 500);
  }
}
