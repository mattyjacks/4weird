...
function toApiRow(row: ServerRow, players: number) {
  ...
  const coinPerMin =
    typeof row.cost_per_min === "number"
      ? row.cost_per_min
      : typeof row.load_per_min === "number"
        ? row.load_per_min
        : 0;
  return {
    id: row.id,
...
export async function GET(req: Request) {
  ...
  if (error) {
    return dbFail("mmorpg/servers GET", error, "Room list is unavailable right now. Try Refresh shortly.");
  }
  ...
  if (ids.length) {
    const { data: sessions, error: sessionError } = await reader
      ...
      .limit(5000);
    if (sessionError) {
      return dbFail("mmorpg/servers GET", sessionError, "Room list is unavailable right now. Try Refresh shortly.");
    }
...
export async function POST(req: Request) {
  ...
  const { data: inserted, error } = await supabase
    ...
    .single();
  if (error || !inserted) {
    return dbFail("mmorpg/servers POST", error, "Room could not be created. No coins moved.");
  }
