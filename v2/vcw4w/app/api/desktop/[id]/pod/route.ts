...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Any control action counts as tending the pod (resets the idle clock).
  await svc.from("desktop_pods").update({ status, last_activity_at: new Date().toISOString() }).eq("id", desktop.id);
