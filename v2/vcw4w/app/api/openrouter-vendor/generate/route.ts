 ...
 * vendor modules (openrouter-chat, openrouter-agent, openrouter-meta).
 * Body: { op, prompt, model?, game_slug?, source? }.
 *
 ...
 * (same rule as fal). meter_openrouter_usage RPC debits first so OpenRouter
 * spend can never leak free; a failed meter fails the run.
 */
...
async function loadRegistry(): Promise<Map<string, OpDef>> {
  ...
  const pairs: [string, string][] = [
    ["@/lib/openrouter-chat", "openrouter-chat"],
    ["@/lib/openrouter-agent", "openrouter-agent"],
    ["@/lib/openrouter-meta", "openrouter-meta"],
  ];
...
export async function POST(req: Request) {
  ...
  const rl = rateLimit(`openrouter-vendor:generate:${data.user.id}`, 20, 60_000);
  ...
  if (registry.size === 0) {
    return fail(
      "OpenRouter vendor catalog unavailable (lib modules not yet present). Try again after the vendor libs land.",
      503,
    );
  }
  if (!def) {
  ...
  if (prompt.length < 3) return fail(`${def.name} needs a prompt (3+ chars).`, 400);

  const game = cleanGameSlug(input.game_slug ?? input.game ?? "lobby");
  if (!/^[a-z0-9-]{1,64}$/.test(game)) return fail("Invalid game_slug.", 400);

  const sourceRaw = String(input.source ?? "openrouter-studio");
  const source =
    sourceRaw === "vcw"
      ? "vcw"
      : sourceRaw === "api"
  ...
  if (!key) {
    return ok({
      ...
      hint: "Set OPENROUTER_API_KEY on the server to queue real OpenRouter runs.",
    });
  }

  // Balance pre-check (402 when short), then the debit lands BEFORE OpenRouter
  // is touched: metering gates the goods, so a failed meter fails the run
  // instead of submitting spend the ledger never sees. The meter RPC re-checks
  // balance under its spend lock, so a race that empties the wallet between
  // check and debit still fails closed with no upstream call submitted.
  try {
    const { data: bal, error: balError } = await supabase.rpc("get_my_coin_balance");
    if (balError) return dbFail("api/openrouter-vendor/generate", balError, "Unable to check balance.");
    if ((Number(bal) || 0) < quote.gross) return fail("Insufficient Vibe Coin balance.", 402);
  } catch (error) {
    return dbFail("api/openrouter-vendor/generate", error, "Unable to check balance.");
  }

  let meter: unknown = null;
  try {
    const { data: result, error } = await supabase.rpc("meter_openrouter_usage", {
      p_vendor: def.vendor,
      p_game: game,
      p_op: opRaw,
      p_qty: qty,
    ...
    if (error)
      return rpcFail("api/openrouter-vendor/generate", error, rpcStatus, "Unable to meter this OpenRouter run.");
    meter = result;
  } catch (error) {
    return dbFail("api/openrouter-vendor/generate", error, "Unable to meter this OpenRouter run.");
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 401 || res.status === 403) {
        console.error(
          `[api/openrouter-vendor/generate] OpenRouter rejected the server key (HTTP ${res.status}, op ${opRaw}).`,
        );
        const refunded = await refundUsage(
          supabase,
          "api/openrouter-vendor/generate",
          "openrouter",
          usageId,
        );
        return fail(
          `OpenRouter key rejected (HTTP ${res.status}).${refunded ? " Charge refunded." : " Charge NOT refunded — contact support."}`,
          502,
        );
      }
      const refunded = await refundUsage(
      ...
      return fail(
        `OpenRouter HTTP ${res.status}: ${text.slice(0, 110)}.${refunded ? " Charge refunded." : " Charge NOT refunded — contact support."}`,
        502,
      );
    }
    ...
    console.error(
      "[api/openrouter-vendor/generate] provider fetch failed",
      String(error instanceof Error ? error.message : error).slice(0, 200),
    );
    const refunded = await refundUsage(
      supabase,
    ...
    return fail(
      `OpenRouter request failed.${refunded ? " Charge refunded." : " Charge NOT refunded — contact support."}`,
      502,
    );
  } finally {
    clearTimeout(timer);
