...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Votes move hot ranking: logged-in bots may vote (member-checked below).
  const botBlock = await requireHuman(req, "POST /api/clans/post/vote", { allowAuthenticated: true });
  ...
  const { data: rpcData, error } = await supabase.rpc("vote_clan_post", {
    p_post_id: id,
    p_value: value,
  });
  if (error) {
    ...
    if (code === "P0001" || code === "") {
      return rpcFail("api/clans/post/[id]/vote POST", error, (msg) => {
    ...
    return dbFail("api/clans/post/[id]/vote POST", error, "Unable to vote.");
  }
