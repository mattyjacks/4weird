...
export default function PlayingGamesPage() {
  return (
    <article>
      <DocsHero
        eyebrow="Docs Â· insert coin"
        title={<>34 cabinets. <span className={theme.title}>One joystick: you.</span></>}
        lede={<>Preserved browser games in isolated play shells â€” guides, cloud saves, leaderboards, guest passes, coin-metered rentals. A 5-hour session on default rates costs about 6 coins.</>}
        stats={[
          ["34", "playable games"],
          ["~6 ðŸª™", "per 5-hour session"],
      ...
      <SectionHead
        ...
        body="The catalog at /games lists all 34 titles. Each game has a detail page (guide link, metadata, play-rate badge, AI badges) and a play page â€” the PlayGate shell around the isolated frame. Add ?match= to a play URL to join a match."
      />
      ...
      <SectionHead
        ...
        body="Signed-in play never shows ads â€” it meters coins instead, with the 25% cut already inside every figure. Two parts:"
      />
      ...
      <ul className="mt-5 list-disc space-y-2 pl-6 text-sm leading-relaxed text-muted-foreground">
        <li><strong className="text-foreground">Load fee (default 1 coin / MiB):</strong> proportional to exact bytes â€” sub-MB loads pay their exact fraction down to 1 centicentcoin. Same version free 24h.</li>
        <li><strong className="text-foreground">Running play (default 1 coin/hr):</strong> billed per second from the first second â€” 100 centicentcoins over 3,600 seconds. 1-minute heartbeats bill only the delta.</li>
        <li><strong className="text-foreground">Developer rates 0â€“100:</strong> mapped devs set their own per-load + per-hour; 0 = free game. The public price list is always visible first.</li>
        <li><strong className="text-foreground">AI bills on top, only when used</strong> â€” see <Link className="underline" href="/docs/game-ai-buddy">Game AI &amp; Buddy</Link>.</li>
      </ul>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        {["SLOT 1", "SLOT 2", "SLOT 3"].map((s, i) => (
          <div key={s} className="rounded-2xl border border-border bg-card p-4 text-center">
            ...
            <p aria-hidden="true" className="mt-1 text-3xl">{i === 2 ? "ðŸš«" : "ðŸ’¾"}</p>
            <p className="mt-1 text-xs text-muted-foreground">{i === 2 ? "Cheat-branded. Still playable, forever flagged." : "â‰¤1 MiB Â· versioned Â· yours"}</p>
          </div>
        ))}
      </div>
      <Callout tone="rose" title="Cheat Mode is a tattoo, not a sticker.">
        Enabling cheats permanently marks that save (<code>cheat_mode:true</code>) as a database invariant â€”
      ...
      <SectionHead
        ...
        body="Gameplay emits aggregate events powering /leaderboards (handles + totals, anonymous-friendly). Telemetry never decides billing â€” the rental session does. Find humans in /lobbies and join via ?match= links; for a permanent home, join a clan."
      />
