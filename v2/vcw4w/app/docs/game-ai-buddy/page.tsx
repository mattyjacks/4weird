...
export default function GameAiBuddyPage() {
  return (
    <article>
      <DocsHero
        eyebrow="Docs Â· your co-pilot"
        title={<>A coach who <span className={theme.title}>watches you play.</span></>}
        lede={<>Dialogue bots, AI directors, voice acting â€” plus a universal 9-voice Gaming Buddy that reads the screen and coaches you live. All metered with the 25% cut included.</>}
        stats={[
          ["9", "voices"],
          ["0.5â€“2x", "speech speed"],
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        <div className="rounded-2xl border border-rose-400/40 bg-gradient-to-b from-rose-500/15 to-transparent p-5">
          ...
          <p className="mt-2 text-sm text-muted-foreground">The core loop needs it â€” e.g. an attack director driving enemies. Playing the game <strong className="text-foreground">is</strong> using AI, metered per turn, decision, or GPU minute.</p>
        </div>
      ...
      <SectionHead
        ...
        title="Buddy 101"
      />
      <MockWindow title="gaming buddy â€” live session" badge="9.2Â¢ this session">
        <div className="space-y-3 text-sm">
          <div className="flex gap-2.5">
            <span aria-hidden="true" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-rose-500/30">ðŸŽ™ï¸</span>
            <div className="rounded-2xl rounded-tl-sm bg-white/10 px-3 py-2">
              <p className="text-[11px] font-bold text-rose-300">Buddy Â· Nova 1.2x</p>
              <p>Nice dodge! Two pickups spawn left in ~5s â€” grab the shield first ðŸ›¡ï¸</p>
            </div>
          </div>
          <div className="flex flex-row-reverse gap-2.5">
            <span aria-hidden="true" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/15">ðŸ™‚</span>
          ...
          <div className="flex flex-wrap gap-2 text-[11px] font-bold text-slate-400">
            <span className="rounded-full border border-white/10 px-2.5 py-1">session 4.1Â¢</span>
            <span className="rounded-full border border-white/10 px-2.5 py-1">total 38.7Â¢</span>
            <span className="rounded-full border border-white/10 px-2.5 py-1">24h 12.0Â¢</span>
            <span className="rounded-full border border-white/10 px-2.5 py-1">1h 4.1Â¢</span>
          </div>
        </div>
      </MockWindow>
      <ol className="mt-5 list-decimal space-y-2 pl-6 text-sm leading-relaxed text-muted-foreground">
        <li>Open any play page, <strong className="text-foreground">start a Buddy session</strong>, ask for what you want: â€œcoach my aim,â€ â€œexplain this level,â€ â€œcall out pickups.â€</li>
        <li><strong className="text-foreground">Keep sessions scoped</strong> â€” one game, one goal. Shorter sessions cost less and coach better.</li>
        <li><strong className="text-foreground">End the session</strong> when you stop playing â€” open sessions keep the door open for metered turns.</li>
      ...
      <Callout tone="rose" title="No voice? Check the stage, not the actor.">
        ...
        speech when provider voice is unavailable â€” the turn still meters. Spend questions? Compare the widget&apos;s session
