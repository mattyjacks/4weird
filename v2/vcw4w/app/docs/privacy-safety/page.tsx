...
export default function PrivacySafetyPage() {
  return (
    <article>
      <DocsHero
        eyebrow="Docs Â· the vault door"
        title={<>Your data. <span className={theme.title}>Your call.</span></>}
        lede={<>What we collect, your self-service rights, how moderation keeps clans safe, and how to report abuse. Full policy at /privacy â€” this guide is the plain-language tour.</>}
        stats={[
          ["13+", "age minimum"],
      ...
      <Steps
        items={[
          ...
          ["Blocked? Fix this", <>Shared clan ownership or active rental escrow blocks deletion with 409 â€” the page names exactly what to resolve.</>],
        ]}
      />
      <Callout tone="emerald" title="Only you can delete you.">
        Self-service deletion is strictly the signed-in holder deleting their own account â€” never anyone else&apos;s. Family
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        {[
          ["ðŸ‘¤ Account", "Email + login, trial/bonus/referral state, coin ledger, usage meters."],
          ["ðŸ•¹ï¸ Play", "Saves (â‰¤1 MiB/slot), aggregate telemetry for leaderboards, rental sessions."],
          ["ðŸ‘¾ Social", "Posts, comments, messages, images (â‰¤1 MB), reactions, roles, reports, wallets."],
          ["ðŸ›¡ï¸ Trust", "Moderation decisions + audit log, privacy requests, anti-fraud signals."],
        ].map(([t, b]) => (
      ...
      <p className="mt-3 text-sm text-muted-foreground">
        ...
        their own terms â€” see <Link className="underline" href="/privacy">Privacy Policy</Link> for categories.
      </p>
      ...
      <div className="mt-5 rounded-3xl border border-rose-400/40 bg-gradient-to-br from-rose-500/15 via-transparent to-transparent p-6">
        <p className="text-lg font-black">ðŸš¨ Reporting abuse â€” including CSAM</p>
        <ol className="mt-3 list-decimal space-y-2 pl-6 text-sm leading-relaxed text-muted-foreground">
          <li>Use the report control on the clan page, post, message, or bot console â€” category + details.</li>
          ...
          <li><strong className="text-foreground">Never repost or describe it</strong> â€” reposting spreads harm and breaks the evidence chain.</li>
      ...
      <div className="mt-5 flex flex-wrap gap-2 text-sm font-bold">
        {["ðŸ”‘ Keys stay secret", "ðŸšª Log out on shared PCs", "ðŸ¤« No passwords in chat", "ðŸ’° Keep clan wallets funded", "ðŸ§¹ Review pending queues", "ðŸ‘¶ 13+ only"].map((t) => (
