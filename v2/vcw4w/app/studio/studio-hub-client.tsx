...
const PICKER_OPTIONS: { value: Picker; label: string; hint: string }[] = [
  { value: "all", label: "All", hint: "Show all 5 studios" },
  ...
  return (
    <div>
      <div className="rounded-xl border border-white/10 bg-white/[.03] p-3">
        <p className="text-xs font-bold text-slate-200">Which studio do I need?</p>
        <div className="mt-2 flex flex-wrap gap-2" role="group" aria-label="Studio picker">
      ...
      {CARDS.length === 0 ? (
        <p role="status" className="mt-3 rounded-xl border border-white/10 bg-white/[.03] p-3 text-sm text-slate-400">
          ...
          {CARDS.map((card) => {
            ...
            return (
              ...
              >
                ...
                <h2 className="mt-2 text-sm font-black tracking-tight">{card.name}</h2>
                <p className="mt-1 line-clamp-2 text-xs leading-snug text-slate-400">{card.purpose}</p>
                <Link
                  href={card.href}
                  tabIndex={active ? undefined : -1}
                  className="mt-3 inline-block rounded-lg border border-cyan-300/40 bg-cyan-300/10 px-3 py-1.5 text-xs font-bold text-cyan-200 transition-colors hover:bg-cyan-300/20"
                >
                  Open →
                </Link>
              </li>
