...
import { cleanDisplayName } from '../../../lib/validate.js';
...
export async function GET(req) {
  ...
  const { data, error } = await userClient(me.accessToken)
    .from('profiles').select('display_name,email,created_at').eq('id', me.user.id).maybeSingle();
...
export async function PATCH(req) {
  ...
  const { error } = await userClient(me.accessToken)
    .from('profiles').update({ display_name: name }).eq('id', me.user.id);
  if (error) return fail(req, 500, 'internal error');
