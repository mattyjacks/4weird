...
(function () {
  ...
  window.FourWeirdServer = {
    // Deliberately narrow generic transport for authenticated first-party
    // account modules. It preserves the same origin allowlist and httpOnly
    // cookie handling as the named methods above.
    request: async function (path, options) {
      var r = await api(String(path || ''), options && { method: options.method, body: options.body });
      return r.data || { success: false, error: r.error };
    },
    signUp: signUp,
    signIn: signIn,
    signOut: signOut,
