(() => {
  type MapId = "desert" | "urban" | "night" | "test";
  type GameMode = "menu" | "countdown" | "race" | "paused" | "finished";
  type MenuView = "main" | "levels" | "garage" | "options";
  type CarId = "starter" | "street" | "gt" | "prototype";
  type PowerupKind = "nitro" | "shield" | "airstrike";
  type RaceTargetId = "player" | number;
  type RaceDirection = "forward" | "reverse";
  ...
  type MapConfig = {
    ...
    sky: string;
    palette: RoadPalette;
    miniMapRoute: readonly MiniMapPoint[];
    sections: readonly RoadSection[];
    decorations: MapDecoration;
    seed: number;
    curvePull: number;
    skyline: "desert" | "urban" | "night";
    laps: number;
    reward: number;
    recommendedCar: CarId;
    opponentCars: readonly CarId[];
  };
  ...
  type GameSave = {
    ...
    sound: boolean;
    bgmVolume: number;
    carSoundVolume: number;
    debugMode: boolean;
    unlimitedNitro: boolean;
    bestPlaces: Partial<Record<MapId, number>>;
  };
  ...
  type Opponent = {
    ...
    avoidanceLockUntil: number;
    blockedBy?: RaceTargetId;
    speed: number;
    baseSpeed: number;
    position: number;
    totalDistance: number;
  ...
  type BlockingTarget = AvoidanceTarget & {
    distance: number;
  };

  type OpponentTrafficIntent = {
    targetOffset: number;
    followSpeed?: number;
    blockedBy?: RaceTargetId;
  };
  ...
  const FIRST_LEVEL_ID: MapId = "desert";
  const SECOND_LEVEL_ID: MapId = "urban";
  const THIRD_LEVEL_ID: MapId = "night";
  const TEST_LEVEL_ID: MapId = "test";
  const SHOW_TITLE_IMAGE_IN_MENUS = false;
  const LEVEL_ORDER: readonly MapId[] = [FIRST_LEVEL_ID, SECOND_LEVEL_ID, THIRD_LEVEL_ID];
  ...
  const POWERUP_KIND_POOL: readonly PowerupKind[] = ["nitro", "nitro", "shield", "airstrike", "shield", "airstrike", "shield"];
  ...
  const ASSETS: readonly AssetMeta[] = [
  ...
  const ASSET_BY_FILE: Record<string, AssetMeta> = Object.fromEntries(
    ASSETS.map((asset) => [asset.file, asset]),
  );
  ...
  const CAR_ORDER: readonly CarId[] = ["starter", "street", "gt", "prototype"];

  const CARS: Record<CarId, CarConfig> = {
    starter: {
      id: "starter",
      label: "Comet 90",
      price: 0,
  ...
  const loadingScreen = requireElement<HTMLElement>("#loadingScreen");
  const loadingPercent = requireElement<HTMLElement>("#loadingPercent");
  const loadingFill = requireElement<HTMLElement>("#loadingFill");
  const menuPanel = requireElement<HTMLElement>("#menuPanel");
  const menuContent = requireElement<HTMLElement>("#menuContent");
  const backButton = requireElement<HTMLButtonElement>("#backButton");
  const resumeButton = requireElement<HTMLButtonElement>("#resumeButton");
  const panelStatus = requireElement<HTMLElement>("#panelStatus");
  const raceMessage = requireElement<HTMLElement>("#raceMessage");
  const raceMessageTitle = requireElement<HTMLElement>("#raceMessageTitle");
  const raceMessageBody = requireElement<HTMLElement>("#raceMessageBody");
  const countdownOverlay = requireElement<HTMLElement>("#countdownOverlay");
  const hudSpeed = requireElement<HTMLElement>("#hudSpeed");
  const hudNitro = requireElement<HTMLElement>("#hudNitro");
  const hudNitroFill = requireElement<HTMLElement>("#hudNitroFill");
  const nitroMeter = requireElement<HTMLElement>("#nitroMeter");
  const hudLap = requireElement<HTMLElement>("#hudLap");
  const hudPlace = requireElement<HTMLElement>("#hudPlace");
  const hudTimer = requireElement<HTMLElement>("#hudTimer");
  const hudRouteItem = requireElement<HTMLElement>("#hudRouteItem");
  const hudRoute = requireElement<HTMLElement>("#hudRoute");
  const raceTimer = requireElement<HTMLElement>("#raceTimer");
  const mapStats = requireElement<HTMLElement>("#mapStats");
  const driveHud = requireElement<HTMLElement>("#driveHud");
  ...
  const touchLeft = requireElement<HTMLButtonElement>("#touchLeft");
  const touchRight = requireElement<HTMLButtonElement>("#touchRight");
  const touchBrake = requireElement<HTMLButtonElement>("#touchBrake");
  const touchNitro = requireElement<HTMLButtonElement>("#touchNitro");

  const keys = new Set<string>();
  const TOUCH_CONTROL_KEYS = ["arrowleft", "arrowright", "arrowdown", "n"] as const;
  ...
  const MAP_BGM_ASSETS: Partial<Record<MapId, string>> = {
  ...
  let selectedMapId: MapId = SECOND_LEVEL_ID;
  let selectedRaceDirection: RaceDirection = "forward";
  let mode: GameMode = "menu";
  let menuView: MenuView = "main";
  ...
  let airstrikes: AirstrikeEffect[] = [];
  ...
  function positiveModulo(value: number, total: number): number {
    return ((value % total) + total) % total;
  }

  function shortestTrackDistance(a: number, b: number): number {
    const raw = Math.abs(a - b);
    return Math.min(raw, trackLength - raw);
  }

  function getPlayerMaxSpeed(): number {
  ...
  function getPlayerDisplayTopSpeed(): number {
    const car = getPlayerCar();
    const nitroTopSpeed = 1 + (NITRO_TOP_SPEED - 1) * car.nitro;
    return car.displayTopSpeed * (player.nitroActive ? nitroTopSpeed : 1);
  }

  function isBuildingAsset(asset: string): boolean {
    return asset.substring(asset.lastIndexOf("/") + 1).startsWith("building_");
  }

  function getCarConfig(id: CarId): CarConfig {
    return CARS[id] ?? CARS.starter;
  }

  function getEquippedCar(): CarConfig {
    return getCarConfig(save.equippedCar);
  }

  function getPlayerCar(): CarConfig {
    return getCarConfig(player.carId);
  }

  function getCarRank(id: CarId): number {
  ...
  function shouldShowDebugMode(): boolean {
    return isLocalhost() && save.debugMode;
  }

  function shouldUseUnlimitedNitro(): boolean {
    return shouldShowDebugMode() && save.unlimitedNitro;
  }

  function createUiButtonClickSoundPool(): HTMLAudioElement[] {
  ...
  function createMapBgmTracks(): Partial<Record<MapId, HTMLAudioElement>> {
    return Object.fromEntries(
      Object.entries(MAP_BGM_ASSETS).map(([mapId, source]) => {
    ...
    ) as Partial<Record<MapId, HTMLAudioElement>>;
  }

  function createCalmBgmTrack(): HTMLAudioElement {
  ...
  function createEngineSound(): HTMLAudioElement {
    const sound = new Audio(ENGINE_SOUND_ASSET);
    sound.loop = false;
    sound.preload = "auto";
    sound.volume = 0;
    const pitchControls = sound as HTMLAudioElement & {
  ...
  function getCarSoundVolume(): number {
    return clamp(save.carSoundVolume, 0, 1);
  }

  function applyMapBgmVolume(): void {
  ...
  function preloadUiButtonClickSounds(): void {
  ...
  function syncEngineLoopPosition(): void {
  ...
  function updateEngineSound(dt: number): void {
    if (mode === "countdown") {
      engineVolume = 0;
      engineRate = ENGINE_IDLE_RATE;
      engineSound.volume = 0;
  ...
  function syncMapBgm(restart = false): void {
  ...
  function shouldPlayUiButtonClick(event: MouseEvent): boolean {
  ...
  function bindUiButtonSounds(): void {
  ...
  function loadSave(): GameSave {
    ...
    try {
      ...
      const ownedCars = Array.isArray(parsed.ownedCars)
        ? parsed.ownedCars.filter((id): id is CarId => CAR_ORDER.includes(id as CarId))
        : fallback.ownedCars;
  ...
  function getMapConfig(id: MapId): MapConfig {
  ...
  function getRaceLaps(): number {
    return getMapConfig(selectedMapId).laps;
  }

  function hasCompletedLevel(id: MapId): boolean {
    const place = save.bestPlaces[id];
    return typeof place === "number" && place > 0;
  }

  function isLevelUnlocked(id: MapId): boolean {
  ...
  function getPreviousLevelName(id: MapId): string {
    const index = LEVEL_ORDER.indexOf(id);
    if (index <= 0) {
      return "";
    }
  ...
  function getSegmentColor(index: number, palette: RoadPalette): SegmentColor {
  ...
  function blankPoint(x: number, y: number, z: number): RoadPoint {
    return {
      world: { x, y, z },
      camera: { x: 0, y: 0, z: 0 },
  ...
  function buildTrack(map: MapConfig, direction: RaceDirection): Segment[] {
    ...
    function addRoad(section: RoadSection): void {
  ...
  function reverseTrack(track: readonly Segment[], map: MapConfig): Segment[] {
  ...
  function decorateTrack(track: Segment[], map: MapConfig): void {
  ...
  function findSegment(position: number): Segment {
    return segments[Math.floor(position / SEGMENT_LENGTH) % segments.length];
  }

  function buildMiniMapPoints(track: readonly Segment[], map: MapConfig, direction: RaceDirection): MiniMapPoint[] {
    if (track.length === 0) {
      return [];
    }
  ...
  function sampleMiniMapRoute(route: readonly MiniMapPoint[], segmentCount: number): MiniMapPoint[] {
    if (route.length < 2) {
      return [{ x: 0.5, y: 0.5 }];
    }
  ...
  function buildSmoothMiniMapRoute(route: readonly MiniMapPoint[]): MiniMapPoint[] {
    const points: MiniMapPoint[] = [];
  ...
  function interpolateCatmullRom(p0: MiniMapPoint, p1: MiniMapPoint, p2: MiniMapPoint, p3: MiniMapPoint, t: number): MiniMapPoint {
    const t2 = t * t;
    const t3 = t2 * t;
    return {
  ...
  function normalizeMiniMapRoute(raw: readonly MiniMapPoint[]): MiniMapPoint[] {
  ...
  function getMiniMapMarker(position: number, offset: number): { x: number; y: number; angle: number } {
  ...
  function project(point: RoadPoint, cameraX: number, cameraY: number, cameraZ: number): void {
  ...
  function getVisualCameraDepth(): number {
  ...
  function createOpponents(map: MapConfig, playerCarId: CarId): Opponent[] {
  ...
  function getOpponentBaseSpeed(map: MapConfig, carConfig: CarConfig, playerCarId: CarId): number {
  ...
  function getOpponentSpeedBalance(mapId: MapId, opponentCarId: CarId, playerCarId: CarId): { maxSpeedRatio: number; aheadSpeedRatio: number } | undefined {
  ...
  function createPowerups(map: MapConfig): Powerup[] {
  ...
  function isPowerupEnabled(kind: PowerupKind): boolean {
    return POWERUP_FLAGS[kind];
  }

  function startRace(mapId = selectedMapId, direction: RaceDirection = selectedRaceDirection): void {
  ...
  function beginRaceAfterCountdown(): void {
    if (mode !== "countdown") {
      return;
    }
  ...
  function awardRaceCoins(place: number): number {
    if (raceRewardPaid) {
      return 0;
    }
  ...
  function createLevelActionButton(label: string, onClick: () => void, enabled: boolean): HTMLButtonElement {
  ...
  function renderGarageMenu(): void {
    const grid = document.createElement("div");
  ...
  function createToggleRow(label: string, enabled: boolean, onClick: () => void): HTMLElement {
  ...
  function createActionRow(label: string, action: string, onClick: () => void): HTMLElement {
    const row = document.createElement("div");
    row.className = "option-row";
    row.innerHTML = `<div><strong>${label}</strong></div>`;
    const button = document.createElement("button");
  ...
  function createRangeRow(label: string, value: number, onInput: (value: number) => void): HTMLElement {
    const row = document.createElement("div");
    row.className = "option-row";
  ...
  function createMenuButton(title: string, detail: string, onClick: () => void, extraClass = ""): HTMLButtonElement {
    const button = document.createElement("button");
    button.type = "button";
  ...
  async function resetOptionsToDefaults(): Promise<void> {
  ...
  function syncFullScreenPreference(): boolean {
  ...
  function updateOpponents(dt: number): void {
  ...
  function getOpponentTrafficIntent(car: Opponent): OpponentTrafficIntent {
  ...
  function getDefaultOpponentTargetOffset(car: Opponent): number {
  ...
  function findBlockingTarget(car: Opponent): BlockingTarget | undefined {
  ...
  function chooseOpponentPassLane(car: Opponent): number | undefined {
  ...
  function getOpponentPassLaneCandidates(car: Opponent): number[] {
  ...
  function isLaneSafeForOpponent(car: Opponent, laneOffset: number): boolean {
  ...
  function getRaceTargetsForAvoidance(car: Opponent): AvoidanceTarget[] {
    const targets: AvoidanceTarget[] = [
      {
        id: "player",
  ...
  function getNearestLaneOffset(offset: number): number {
    return LANE_OFFSETS[getNearestLaneIndex(offset)];
  }

  function getNearestLaneIndex(offset: number): number {
    let bestIndex = 0;
  ...
  function getTurnLimitedSpeed(speed: number, turnAmount: number): number {
    return speed * (1 - clamp(turnAmount, 0, 1) * MAX_TURN_SPEED_REDUCTION);
  }

  function getOpponentTargetSpeed(car: Opponent, turnAmount: number, followSpeed?: number): number {
  ...
  function collectPowerup(item: Powerup): void {
    if (!isPowerupEnabled(item.kind)) {
      return;
    }
  ...
  function updateAirstrikes(): void {
  ...
  function triggerAirstrike(): void {
    if (!POWERUP_FLAGS.airstrike) {
      return;
    }
  ...
  function applyAirstrikeHit(strike: AirstrikeEffect): void {
  ...
  function getStandings(): Array<{ id: RaceTargetId; distance: number }> {
    const standings: Array<{ id: RaceTargetId; distance: number }> = [
  ...
  function getTargetState(id: RaceTargetId):
    | {
      position: () => number;
      offset: () => number;
      speed: () => number;
  ...
  function resolveCollisions(
    previousPlayerDistance: number,
    previousPlayerOffset: number,
    previousOpponentDistances: readonly number[],
    previousOpponentOffsets: readonly number[],
  ): void {
  ...
  function resolvePlayerOpponentCollisions(
    previousPlayerDistance: number,
    previousPlayerOffset: number,
    previousOpponentDistances: readonly number[],
    previousOpponentOffsets: readonly number[],
    ...
    for (const car of opponents) {
      ...
      const lateralCollision = lateralPathsOverlap(
        previousPlayerOffset,
        player.x,
        previousCarOffset,
        car.offset,
        (PLAYER_ROAD_WIDTH + OPPONENT_ROAD_WIDTH) * 0.5,
      );
  ...
  function resolveOpponentCollisions(previousOpponentDistances: readonly number[], previousOpponentOffsets: readonly number[]): void {
  ...
  function lateralPathsOverlap(previousA: number, currentA: number, previousB: number, currentB: number, clearance: number): boolean {
  ...
  function placeOpponentBehindPlayer(car: Opponent): void {
    car.totalDistance = Math.max(0, player.totalDistance - COLLISION_SEPARATION);
    car.position = positiveModulo(car.totalDistance, trackLength);
  }

  function placePlayerBehindOpponent(car: Opponent): void {
    player.totalDistance = Math.max(0, car.totalDistance - COLLISION_SEPARATION);
    player.position = positiveModulo(player.totalDistance, trackLength);
    player.lap = clamp(Math.floor(player.totalDistance / trackLength) + 1, 1, getRaceLaps());
  }

  function placeOpponentBehindOpponent(rear: Opponent, front: Opponent): void {
  ...
  function applyOpponentRearEndPenalty(car: Opponent, frontSpeed: number): void {
  ...
  function calculatePlace(): number {
    let place = 1;
    for (const car of opponents) {
  ...
  function drawMiniMap(map: MapConfig): void {
    ...
    const toX = (value: number) => value * width;
    const toY = (value: number) => value * height;
  ...
  function drawMiniMapFinishLine(scale: number): void {
    ...
    for (let row = 0; row < rows; row += 1) {
      for (let column = 0; column < columns; column += 1) {
        ...
        mapCtx.fillRect(
          -thickness / 2 + column * cellWidth,
          -span / 2 + row * cellHeight,
          cellWidth,
          cellHeight,
        );
      }
  ...
  function drawMiniMapDot(x: number, y: number, radius: number, color: string): void {
  ...
  function drawMiniMapPlayer(x: number, y: number, angle: number, size: number): void {
    const px = x * miniMapCanvas.width;
    const py = y * miniMapCanvas.height;
  ...
  function drawBackground(map: MapConfig, time: number): void {
  ...
  function drawSkyline(map: MapConfig, horizon: number, time: number): void {
  ...
  function renderRoad(map: MapConfig): void {
    ...
    let finalLapFinishSegment: Segment | undefined;
    ...
    for (let n = 0; n < DRAW_DISTANCE; n += 1) {
      ...
      if (
        segment.p1.camera.z <= getVisualCameraDepth() ||
  ...
  function drawSegment(segment: Segment): void {
  ...
  function getFinishLineSegmentOffset(segmentIndex: number): number {
  ...
  function drawFinishLine(segment: Segment, segmentOffset: number): void {
    const p1 = segment.p1.screen;
    const p2 = segment.p2.screen;
  ...
  function drawFloatingFinishText(segment: Segment): void {
    const p1 = segment.p1.screen;
    const p2 = segment.p2.screen;
  ...
  function drawSpritesAndCars(baseIndex: number): void {
  ...
  function getVisibleCars(): VisibleCar[] {
  ...
  function getVisiblePowerups(): VisiblePowerup[] {
    const visible: VisiblePowerup[] = [];
    const drawLimit = DRAW_DISTANCE * SEGMENT_LENGTH;
  ...
  function getProjectedPoint(position: number, offset: number, nearClip = getVisualCameraDepth()): ProjectedPoint | undefined {
  ...
  function drawRoadSprite(sprite: RoadSprite, segment: Segment): void {
  ...
  function drawPowerup(visible: VisiblePowerup): void {
  ...
  function getPowerupSize(kind: PowerupKind, point: ProjectedPoint): number {
  ...
  function drawNitroPowerup(x: number, y: number, size: number, color: string): void {
  ...
  function drawPowerupSymbol(kind: PowerupKind, x: number, y: number, size: number, color: string): void {
    ctx.strokeStyle = color;
    ctx.fillStyle = color;
    ctx.lineWidth = Math.max(2, size * 0.08);
    ctx.lineCap = "round";
  ...
  function drawTrackImpactEffects(): void {
  ...
  function drawTrackAirstrikes(): void {
    if (!POWERUP_FLAGS.airstrike) {
      return;
    }
  ...
  function drawOpponentCar(visible: VisibleCar): void {
    ...
    const maxWidth = Math.min(
      renderWidth * OPPONENT_MAX_SCREEN_WIDTH_RATIO,
      OPPONENT_MAX_SCREEN_WIDTH,
      getPlayerCarDisplayWidth() * OPPONENT_MAX_PLAYER_WIDTH_RATIO,
    );
  ...
  function drawPlayerCar(time: number): void {
  ...
  function getPlayerDisplaySpeed(): number {
    const currentMaxSpeed = getPlayerMaxSpeed();
    const displayTopSpeed = getPlayerDisplayTopSpeed();
    return (player.speed / Math.max(1, currentMaxSpeed)) * displayTopSpeed;
  }

  function getOpponentDisplaySpeed(car: Opponent): number {
    const config = getCarConfig(car.carId);
    const baseMaxSpeed = BASE_MAX_SPEED * config.maxSpeed;
    return (car.speed / Math.max(1, baseMaxSpeed)) * config.displayTopSpeed;
  }

  function drawDebugSpeedLabel(speed: number, centerX: number, baselineY: number, carWidth: number): void {
  ...
  function getPlayerCarRect(time: number): { x: number; y: number; width: number; height: number } {
  ...
  function getPlayerCarDisplayWidth(): number {
  ...
  function drawCarAfterImages(asset: string, x: number, y: number, width: number, height: number, clipY = renderHeight, strength = 1): void {
    ...
    for (let index = copies; index >= 1; index -= 1) {
      ...
      ctx.drawImage(
        ...
        trailY,
        trailWidth,
        Math.min(trailHeight, clipY - trailY),
      );
    }
  ...
  function drawShieldBubble(cx: number, cy: number, rx: number, ry: number, strength: number): void {
  ...
  function drawImpact(x: number, y: number, size: number, effect: ImpactEffect): void {
  ...
  function drawAirstrikeWarning(x: number, y: number, size: number, strike: AirstrikeEffect): void {
  ...
  function drawPlayerAirstrikeAndImpactEffects(time: number): void {
    const rect = getPlayerCarRect(time);
  ...
  function drawAsset(asset: string, x: number, y: number, width: number, height: number, clipY = renderHeight): void {
  ...
  function updateVisualPlayerX(dt: number): void {
    if (mode !== "race") {
      visualPlayerX = player.x;
      return;
    }
  ...
  function updateNitroZoomVisual(dt: number): void {
    if (mode === "paused") {
      return;
    }
  ...
  function bindTouchButton(button: HTMLButtonElement, key: string): void {
