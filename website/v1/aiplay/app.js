/* ==========================================================================
   4WEIRD AIPLAY // ANTIGRAVITY 2.0 MELDED APPLICATION LOGIC
   ========================================================================== */
...
export class Analytics {
      ...
      try {
        ...
        try {
          if (el.gameIframe.contentWindow) {
            el.gameIframe.contentWindow.postMessage({ type: 'AIPLAY_CLICK', x: targetX, y: targetY }, '*');
          }
        } catch(e){}
      }
  ...
  function initAuthCheck() {
    ...
    if (typeof process !== 'undefined' && process.env && process.env.AIPLAY_AUTH_PASSWORD) {
      requiredPassword = process.env.AIPLAY_AUTH_PASSWORD;
    } else if (window.AIPLAY_AUTH_PASSWORD) {
      requiredPassword = window.AIPLAY_AUTH_PASSWORD;
    } else {
      const params = new URLSearchParams(window.location.search);
      if (params.has('AIPLAY_AUTH_PASSWORD')) {
        requiredPassword = params.get('AIPLAY_AUTH_PASSWORD');
      } else if (params.get('auth') === 'true' || params.has('lock')) {
        requiredPassword = localStorage.getItem('AIPLAY_AUTH_PASSWORD') || '4weird2026';
      } else if (localStorage.getItem('AIPLAY_AUTH_PASSWORD')) {
        requiredPassword = localStorage.getItem('AIPLAY_AUTH_PASSWORD');
      }
    }

    if (requiredPassword) {
      state.authPassword = requiredPassword;
      const isAuth = sessionStorage.getItem('aiplay_authenticated') === 'true';
      if (!isAuth) {
  ...
  function handleAuthSubmit(e) {
    ...
    const targetPassword = state.authPassword || localStorage.getItem('AIPLAY_AUTH_PASSWORD') || '4weird2026';

    if (inputVal === targetPassword || !targetPassword) {
      sessionStorage.setItem('aiplay_authenticated', 'true');
      if (el.authOverlay) el.authOverlay.classList.add('hidden');
      ...
      synth.playSuccess();
      log('[SECURITY] Password authentication successful. AIPlay Sandbox unlocked.', 'success');
    } else {
      if (el.authErrorMsg) {
    ...
    if (el.btnAuthLock) {
      el.btnAuthLock.addEventListener('click', () => {
        ...
        if (isAuth && state.authPassword) {
          if (confirm('Lock AIPlay session with environment password?')) {
            sessionStorage.removeItem('aiplay_authenticated');
            initAuthCheck();
          }
        } else if (!state.authPassword) {
          const p = prompt('Set simulated Vercel environment password (process.env.AIPLAY_AUTH_PASSWORD):', '4weird2026');
          if (p) {
            localStorage.setItem('AIPLAY_AUTH_PASSWORD', p);
            sessionStorage.removeItem('aiplay_authenticated');
            initAuthCheck();
          }
        } else {
  ...
  function triggerDetectedBug(customBug = null) {
    ...
    if (isTauriRuntime()) {
      invokeTauriCommand('show_native_notification', {
        title: `[4WEIRD AIPLAY] Defect Caught: ${bug.id}`,
...
`;
    ...
    md += `\n*End of Bug Report Artifact. Compiled by 4WEIRD AIPlay + Antigravity 2.0 Melded Suite.*`;
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    if (isTauriRuntime()) {
      ...
      console.log('[4WEIRD AIPLAY] Running inside Tauri Desktop Shell!');
